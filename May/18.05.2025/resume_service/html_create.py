...
def generate_html(resume_data: dict):
    if not resume_data:
        ...
        <html>
        <body>
        <h1>Ошибка, данные для генерации страницы не найдены </h1>
        </body>
        </html>
        """
        return output
   ...
   """
        ...
        for edu in resume_data["education"]:
            html += f"""
                <div class="item">
                    ...
                    <div class="date"> {edu.get("Годы","")}</div>
                    <p>{edu.get("Специальность","")}</p>
                </div>
            """
        # Закрываем div class section
        html += """ 
        </div>
        """
        ...
        for exp in resume_data["experience"]:
            html += f"""
                <div class="item">
                    ...
                    <div class="date"> {exp.get("Годы","")}</div>
                    <p>{exp.get("Специальность","")}</p>
                </div>
            """
        # Закрываем div class section
        html += """ 
    ...
    if "skills" in resume_data:
        ...
        html += """
                ...
                <div class="footer">
                    Создано с помощью программы "Генератора резюме" 2025г.
                </div>
