...
import xlsxwriter  # Новая библиотека для создания Excel-файлов
...
# НОВОЕ: Шаг 7: Сохраняем данные в Excel-файл с форматированием
print("Создаём Excel-файл...")

# Создаём новый Excel-файл
workbook = xlsxwriter.Workbook('quotes.xlsx')  # Имя файла: quotes.xlsx
worksheet = workbook.add_worksheet()  # Добавляем лист (страницу в Excel)

# Настраиваем форматы (стили для текста)
# Формат для заголовков: жирный, центр, светло-голубой фон
header_format = workbook.add_format({
    'bold': True,  # Жирный шрифт
    'align': 'center',  # Выравнивание по центру
    'valign': 'vcenter',  # Вертикальное выравнивание по центру
    'bg_color': '#ADD8E6'  # Светло-голубой цвет фона (можно выбрать другой по ссылке)
})

# Формат для данных: выравнивание слева
data_format = workbook.add_format({
    'align': 'left',  # Выравнивание по левому краю
    'valign': 'vcenter'  # Вертикальное по центру
})

# Формат для чётных строк: светло-серый фон (чередование)
...
# Пишем заголовки (первая строка)
worksheet.write('A1', 'Автор', header_format)  # Столбец A: Автор
worksheet.write('B1', 'Цитата', header_format)  # Столбец B: Цитата
worksheet.write('C1', 'Теги', header_format)  # Столбец C: Теги
worksheet.write('D1', 'Ссылка на автора', header_format)  # Столбец D: Ссылка

# Пишем данные (начиная со второй строки)
for row_num, result in enumerate(results, start=1):  # row_num — номер строки, начиная с 1
    # Выбираем формат: чётная строка — серый фон, нечётная — обычный
    row_format = even_row_format if row_num % 2 == 0 else data_format
    
    # Пишем данные в ячейки
    worksheet.write(f'A{row_num + 1}', result['author'], row_format)  # Автор в столбце A
    worksheet.write(f'B{row_num + 1}', result['quote'], row_format)  # Цитата в B
    worksheet.write(f'C{row_num + 1}', ', '.join(result['tags']), row_format)  # Теги в C (соединяем запятой)
    worksheet.write(f'D{row_num + 1}', result['author_url'], row_format)  # Ссылка в D

# Настраиваем ширину столбцов
worksheet.set_column('A:A', 20)  # Столбец A (Автор): ширина 20
worksheet.set_column('B:B', 50)  # Столбец B (Цитата): ширина 50
worksheet.set_column('C:C', 30)  # Столбец C (Теги): ширина 30
worksheet.set_column('D:D', 40)  # Столбец D (Ссылка): ширина 40

# Закрываем Excel-файл (сохраняем)
workbook.close()

print("Excel-файл quotes.xlsx создан!")

# Шаг 8: Закрываем браузер
driver.quit()

print("\nГотово! Данные сохранены в quotes.json и quotes.xlsx")  # Финальное сообщение
